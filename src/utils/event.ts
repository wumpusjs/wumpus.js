...
export async function loadEvents(client: Client) {
	...
	events.files.forEach((event) => {
		...
		if (exportedContent.once) {
			client.once(exportedContent.event as any, (...args: any[]) => exportedContent.execute(...args));
		} else {
			client.on(exportedContent.event as any, (...args: any[]) => exportedContent.execute(...args));
		}
	});
}